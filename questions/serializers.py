from questions.models import Question, Answer, Reaction, Reply,\
    QuestionReaction, Tag, QuestionTag
...
class QuestionReactionSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        ...
        fields = ('question', 'reaction', 'author', 'author',
                  'author_name', 'reaction_name', 'reaction_score')


class TagSerializer(serializers.HyperlinkedModelSerializer):
    # questions = serializers.HyperlinkedRelatedField
    # (view_name='tags-question-detail', read_only=True)
    questions = serializers.HyperlinkedIdentityField(
        view_name='tag-question-detail', lookup_field='name', read_only=True)
    stories = serializers.HyperlinkedIdentityField(
        view_name='tag-story-detail', lookup_field='name', read_only=True)
...
class QuestionSerializer(serializers.HyperlinkedModelSerializer):
    answers = serializers.HyperlinkedIdentityField(
        view_name='question-answers')
    # author_name = serializers.ReadOnlyField
    # (source='author.username', read_only=True)
    # author_id = serializers.ReadOnlyField(source='author.id', read_only=True)
    tags = QuestionTagSerializer(source='questiontag_set', many=True)
    reactions = QuestionReactionSerializer(
        source='questionreaction_set', many=True)

    class Meta:
        many = True
        model = Question
        fields = ['url', 'id', 'author', 'question', 'description',
                  'created_at', 'modified_at', 'answers', 'reactions', 'tags']
